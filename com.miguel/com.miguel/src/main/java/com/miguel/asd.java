 ...
 */
public class asd extends AbstractWorker<asdTask, asdResult> {

    /**
     * Logger for logging purposes.
     */
    private static final Logger LOG = LoggerFactory.getLogger(asd.class);
    ...
    public asd(final asdTask task, final DataStore dataStore, final String outputQueue, final Codec codec, final long resultSizeThreshold) throws InvalidTaskException {
    ...
    public WorkerResponse doWork() throws InterruptedException, TaskRejectedException {
        asdResult result = processFile();
        if(result.workerStatus == asdStatus.COMPLETED){
     ...
     */
    private asdResult processFile() throws InterruptedException {
        ...
        try {
            ...
            if(getTask().action == asdAction.REVERSE){
                for(int i=original.length()-1; i>=0; i--){
                    result = result + original.charAt(i);
                }
            } else if(getTask().action == asdAction.CAPITALISE){
                result = original.toUpperCase();
            } else if(getTask().action == asdAction.VERBATIM){
                result = original;
            }
            ...
            asdResult workerResult = new asdResult();
            workerResult.workerStatus = asdStatus.COMPLETED;
            ...
            return createErrorResult(asdStatus.SOURCE_FAILED);
        } catch (DataStoreException e) {
            //DataStoreException thrown when storing data in the datastore
            LOG.warn("Error storing result", e);
            return createErrorResult(asdStatus.STORE_FAILED);
        } catch (IOException e) {
            //IOException thrown if the conversion from InputStream to String fails
            LOG.warn("Error converting input stream to text", e);
            return createErrorResult(asdStatus.WORKER_FAILED);
        }
    }

    /**
     * If an error in the worker occurs, create a new asdResult with the corresponding worker failure status.
     */
    private asdResult createErrorResult(asdStatus status){
        asdResult workerResult = new asdResult();
